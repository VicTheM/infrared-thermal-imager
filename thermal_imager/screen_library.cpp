...
void TFT_22_ILI9225::drawBitmap(int16_t x, int16_t y, uint8_t *bitmap, int16_t w, int16_t h) {
    _drawBitmap( x,  y, (const uint8_t*)bitmap,  w,  h, 0,  0, false, false, false );
}
...
void TFT_22_ILI9225::_drawBitmap(int16_t x, int16_t y, const uint8_t *bitmap, int16_t w, int16_t h,
                                 uint16_t color, uint16_t bg, bool transparent, bool progmem, bool Xbit) { 
    int16_t i, j;
    int16_t wx0, wy0, wx1, wy1, wh;
    ...
    SPI_DC_HIGH();  // Set DC high for data mode
    SPI_CS_LOW();   // Select the display

    for (j = 0; j < wh; j++) {
        for (i = 0; i < w; i++) {
            uint16_t pixelColor;

            pixelColor = ((uint16_t)bitmap[(j * w + i) * 2] << 8) | bitmap[(j * w + i) * 2 + 1];
            HSPI_WRITE16(pixelColor);
        }
    }

    SPI_CS_HIGH(); // Deselect display
}
