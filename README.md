...
<p>
This projects aims to build a devices that generates images of the internal part of the human body (to be specific - the neck) using an array of thermal sensors, these images will be processed by an
...
[!WARNING] If more than 10 images are on the device they will all be automatically deleted to store new images
---
...
[!NOTE] A max of 10 high quality images can be saved locally on the device, capturing more than 10 images at a time will lead to loss of the previous 10 images, so try to download the images before capturing again.
...
*Capture button left leg --- D19*
*Capture button right leg --- GND* (with a 5.7k pull-down resistor)
*Server switch left leg --- D18*
*Server switch right leg --- GND* (with a 5.7k pull-down resistor)
*LED --- D12* (with a 220 ohm resistor)
